...
class new_model_cls_2(nn.Module) :
    ...
    def forward(self,x): # x: image_rgb : [x, 9, 512, 512]
        dn = self.model_dn(x)
        con=torch.cat([x,dn], dim=1)
        output = self.front(con)  # output : [x, 3, 512, 512]
        output = self.model_cls(output)
        return output
...
def main():
    ...
    if "state_dict" in state_dict_cls.keys():
        ...
        model_cls_refine.load_state_dict(state_dict_cls.state_dict())
    else:
        model_cls_refine.load_state_dict(state_dict_cls)
    ...
    for i in tqdm(range(len(all_file_names))):
        ...
        predict_bmi_scaled = model_cls_refine(input_tensor)
    ...
    predict_df.to_excel('/home/ylab3/HRnet_BMI_estimator/9_channel/HRnet_48_aug_420_Predict_method_1_linear.xlsx')
