...
class new_model_cls(nn.Module) :
    def __init__(self,model_cls):
        ...
        nn.BatchNorm2d(9, eps=1e-05, momentum=0.01, affine=True, track_running_stats=True),
        nn.ReLU(inplace=True),
        nn.Conv2d(9, 3, kernel_size=(1, 1), stride=(1, 1)),
        )
        self.model_cls = model_cls # [x, 6, 512, 512]

    def forward(self,x): # x: image_rgb : [x, 9, 512, 512]
        x = self.front(x)  # output : [x, 3, 512, 512]
        x = self.model_cls(x)
        return x
...
def main():
    ...
    model_cls_fined=new_model_cls(model_cls)
    # model_cls_fined = nn.DataParallel(model_cls_fined, device_ids=gpus).cuda()
    ...
    if "state_dict" in state_dict_cls.keys():
        ...
        model_cls_fined.load_state_dict(state_dict_cls.state_dict())
    else:
        model_cls_fined.load_state_dict(state_dict_cls)
    ...
    df=pd.read_csv('dataset/BMI/Image_test.csv', header=None)
    df[0] = df[0]+'.jpg'
    all_file_names=list(df[0])
    ...
    # plt.imsave("test_full_depth_.jpeg", reNormalize(output_depth, mean, std))
        ...
        predict_bmi_scaled = model_cls_fined(cls_input)
    ...
    predict_df.to_excel('/home/ylab3/HRnet_BMI_estimator/9_channel/HRnet_48_pretrained_360_GT_Predict_paper.xlsx')
